...
```c#
var batch1 = bitpay.GetPayoutBatch(batch.Id);
```
...
```c#
var batches = bitpay.GetPayoutBatches(startDate, endDate, PayoutStatus.New);
```
...
```c#
var deleteBatch = bitpay.CancelPayoutBatch(batch.Id);
```
...
var result = bitpay.requestPayoutBatchNotification(batch.Id);
```
